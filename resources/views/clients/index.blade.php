<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            <h2 class="text-3xl font-semibold tracking-tight text-slate-950">Clients</h2>
            <p class="text-sm text-slate-500">Ajoutez des clients et retrouvez-les directement dans la meme page.</p>
        </div>
    </x-slot>
    ...
    <div class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <section class="panel-soft p-6 sm:p-8">
            <div class="mb-6">
                <h3 class="section-title">Formulaire client</h3>
                <p class="section-copy">Creez un client puis retrouvez-le directement dans la liste.</p>
            </div>
            ...
            <form method="POST" action="{{ route('clients.store') }}" class="grid gap-5 md:grid-cols-2">
                @csrf
                <div>
        ...
        <section class="panel-soft overflow-hidden">
            <div class="border-b border-slate-200 px-6 py-5">
                ...
                <p class="section-copy">Chaque client ajoute apparaitra automatiquement ici.</p>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse($clients as $client)
                            <tr>
                                <td class="font-medium text-slate-900">{{ $client->nom }} {{ $client->prenom }}</td>
                                <td>{{ $client->telephone ?: '-' }}</td>
                                <td>{{ $client->email ?: '-' }}</td>
                                <td>{{ $client->ventes_count }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-sm text-slate-500">Aucun client enregistre.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
